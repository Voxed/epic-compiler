...
/**
 * Base visitor will throw unimplemented for the entire syntax to allow for
 * partial visitors.
 */
class BaseVisitor : public parser::Visitor
{
  public:
    explicit BaseVisitor(const char* visitorName);
    ...
    void visitStruct(parser::Struct* p) override;
  ...
  private:
    /**
     * Throw UnimplementedException with a neat message.
     */
    void unimplemented(const char* syntaxName);
